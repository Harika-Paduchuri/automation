...
def pytest_sessionfinish(session, exitstatus):
    ...
    if os.path.exists(report_path):
        ...
        print("HTML report not found. Generate it using pytest --html=report.html")
...
def pytest_addoption(parser):
    ...
    parser.addoption("--base-url", default="https://opensource-demo.orangehrmlive.com/web/index.php/auth/login", help="Base URL for tests")
...
# ----------------------------------------------------------------------------
# STEP 6: AUTO-GENERATE ALLURE REPORT AFTER TEST RUN
# ----------------------------------------------------------------------------
def pytest_sessionfinish(session, exitstatus):
    """
    After the full pytest session completes, generate the Allure HTML report
    from the results directory so the report is ready immediately.
    """
    ...
    if not allure_results.exists() or not any(allure_results.iterdir()):
        print("\n[ALLURE] No Allure results found in reports/allure-results. Skipping report generation.")
        return

    allure_executable = shutil.which("allure") or shutil.which("allure.cmd")
    if allure_executable is None:
        print("\n[ALLURE] Allure CLI not found on PATH. Install Allure and run 'allure generate' manually.")
        return

    try:
        print(f"\n[ALLURE] Generating report from {allure_results} to {allure_report}")
        command = f'"{allure_executable}" generate "{allure_results}" -o "{allure_report}" --clean'
        subprocess.run(command, shell=True, check=True)
        print(f"[ALLURE] Report generated successfully: {allure_report}")
    except Exception as exc:
        print(f"[ALLURE] Failed to generate Allure report: {exc}")
